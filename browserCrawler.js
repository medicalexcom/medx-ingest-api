...
import { chromium } from '@playwright-extra/playwright-extra';
import StealthPlugin from '@extra/stealth-plugin';
...
async function autoExpand(page) {
  for (let pass = 0; pass < 3; pass++) {
    for (const sel of CLICK_SELECTORS) {
      let elements = [];
      try { elements = await page.$$(sel); } catch { /* ignore engine-specific */ }
    ...
    for (const w of WAITERS) {
      ...
      try { await page.locator(w).first().waitFor({ timeout: 1200 }); } catch { /* ignore */ }
    }
...
async function collectSections(page) {
  return page.evaluate(() => {
    ...
    const map = {
      ...
      features: [
        '.features','#features','section#features','.feature-list','.product-features',
        '#key-features','#feature-highlights','.benefits','.feature-benefits',
        '.highlight-list', 'h2:has(+ ul)', 'h3:has(+ ul)'
      ],
      included: [
        'text="What’s in the box"','text="Included Items"','text="Product Includes"',
        'text="Package Includes"','.included','.includes','.in-the-box','.box-contents',
    ...
    if (!sections.included) {
      ...
      for (const p of document.querySelectorAll(sel)) {
        ...
        if (txt.includes("what’s in the box") || txt.includes("included items") || txt.includes("product includes") || txt.includes("package includes")) {
    ...
    // Definition lists
    const dls = Array.from(document.querySelectorAll('dl')).map(dl => dl.innerText.trim());
...
/**
 * Collect links on the page: anchors, product-like images, PDFs, and JSON resources.
 * Returns a shape compatible with Code B, plus extra fields under links_extra.
 * @param {import('playwright').Page} page
 */
async function collectLinks(page, { productImageLimit = 2 } = {}) {
  return page.evaluate(({ productImageLimit }) => {
    const anchorsAll = Array.from(document.querySelectorAll('a[href]'))
      ...
      .filter(Boolean)
      .map(h => {
        try { return new URL(h, document.baseURI).href; } catch { return null; }
      })
      .filter(Boolean);
    ...
    const imgsAll = Array.from(document.images)
      ...
      .filter(Boolean)
      .map(s => {
        try { return new URL(s, document.baseURI).href; } catch { return null; }
      })
      .filter(Boolean);
    ...
    candidateImages = candidateImages.filter(src => {
      const n = getName(src);
      // keep if seen across caches (count>1) OR name doesn't look like accessory -\d
    ...
    const productImages = Object.values(imagesByName).slice(0, productImageLimit);
...
async function collectSeoMeta(page) {
  return page.evaluate(() => {
    const pick = (selector, attr) => document.querySelector(selector)?.getAttribute(attr) || null;
    const pickContent = (selector) => document.querySelector(selector)?.getAttribute('content') || null;
...
/**
 * Collect inline data blobs: application/json scripts and common window globals.
 * @param {import('playwright').Page} page
 */
async function collectInlineData(page) {
  return page.evaluate(() => {
    ...
    WIN_KEYS.forEach(k => {
      try {
        if (typeof window[k] !== 'undefined') out.window_vars[k] = window[k];
      } catch { out.window_vars[k] = null; }
    });
    if (Array.isArray(window.dataLayer)) out.window_vars.dataLayer = window.dataLayer;

    // Next.js embedded script by id
    const nextEl = document.querySelector('#__NEXT_DATA__');
    if (nextEl && nextEl.textContent) {
      try { out.window_vars.__NEXT_DATA__ = JSON.parse(nextEl.textContent); } catch { /* ignore */ }
    }
...
async function collectLinkHints(page) {
  return page.evaluate(() => {
    const HINTS = [
      'manual', 'datasheet', 'spec sheet', 'specification sheet', 'brochure', 'catalog',
      'ifu', 'instructions', 'user guide', 'quick start', 'installation', 'sds', 'msds',
    ...
    for (const a of anchors) {
      ...
      const match = HINTS.find(h => text.includes(h) || file.includes(h.replace(/\s+/g, '')));
...
async function collectImagesWithAlt(page) {
  return page.evaluate(() => {
    ...
    return imgs.map(img => ({
      src: (() => { try { return new URL(img.src, document.baseURI).href; } catch { return img.src || null; } })(),
      alt: img.alt || null,
...
/**
 * Browse a product URL with Playwright, expand content, and extract a comprehensive payload.
 *
 * @param {string} url
 * @param {object} [opts]
 * @param {number} [opts.navigationTimeoutMs=30000]
 * @param {string} [opts.userAgent='Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120 Safari/537.36']
 * @param {{width:number,height:number}} [opts.viewport={width:1366,height:900}]
 * @param {boolean} [opts.headless=true]
 * @param {boolean} [opts.enforceEnglishHeuristic=true]   // apply ASCII-heavy line filtering
 * @param {number}  [opts.productImageLimit=2]            // cap filtered product images in links.images
 * @param {boolean} [opts.includeHtml=false]              // attach full_html (debug)
 */
export async function browseProduct(url, opts = {}) {
  ...
  try {
    ...
    // Clean sections (and visible text) with English-line heuristic if enabled
